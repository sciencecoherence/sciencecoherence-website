...
$roomVersion = 4
try {
    $serverPyPath = Join-Path $roomDirectory 'server.py'
    if (Test-Path -LiteralPath $serverPyPath) {
        $foundVersion = Select-String -Path $serverPyPath -Pattern '^ROOM_VERSION\s*=\s*(\d+)' | Select-Object -First 1
        if ($foundVersion -and $foundVersion.Matches.Groups[1].Value) {
            $roomVersion = [int]$foundVersion.Matches.Groups[1].Value
        }
    }
} catch { }
try {
