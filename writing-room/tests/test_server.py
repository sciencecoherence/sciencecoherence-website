...
class WritingRoomTests(unittest.TestCase):
    ...
    def make_pages(self):
        import shutil
        tools = self.website / 'tools'
        tools.mkdir(exist_ok=True)
        shutil.copy(Path(__file__).resolve().parents[2] / 'tools' / 'build-documents.py', tools / 'build-documents.py')
        self.pages = [{'id':'home','name':'Home','route':'#/','fields':[
                          {'key':'hero.title','group':'Opening','label':'Title','kind':'line','value':'Nothing is<br>outside.'},
                          {'key':'hero.text','group':'Opening','label':'Paragraph','kind':'text','value':'One loop.'}]},
                      {'id':'research','name':'Research','route':'#/research','section':'research','fields':[]},
                      {'id':'protocol','name':'The Protocol','route':'#/protocol','document':'doc','fields':[
                          {'key':'banner.title','group':'Closing banner','label':'Title','kind':'line','value':'The architecture.'}]}]
        self.pages_raw = b'window.SC_PAGES = ' + room_module.encoded(self.pages) + b';\n'
        (self.website / 'pages.js').write_bytes(self.pages_raw)
        (self.website / 'documents').mkdir()
        self.doc_raw = ('<!-- A comment that names <section data-id …> must never be read as a section. -->\n'
                        '<script type="application/json" id="meta">\n{"id": "doc", "title": "The document", "version": "1.0", "date": "2026-09-11",'
                        ' "facts": [{"value": "18h", "label": "Daily fast"}]}\n</script>\n'
                        '<section data-id="overview" data-num="0" data-label="Overview" data-title="Start" data-summary="How to read it.">\n'
                        '  <p>First words.</p>\n  <div class="pacer" id="pacer"><button id="pacer-toggle">Start</button></div>\n</section>\n'
                        '<section data-id="body" data-num="1" data-label="Body" data-title="The body" data-summary="Second.">\n  <p>Second words.</p>\n</section>\n').encode()
        (self.website / 'documents' / 'doc.html').write_bytes(self.doc_raw)
        return self.room.state()

    def page_payload(self, state, page, **values):
        return {'page':page,'values':values,'revisions':state['pageRevisions']}

    def test_pages_offer_their_own_section_collection_and_document_fields(self):
        state = self.make_pages()
        pages = {p['id']: p for p in state['pages']}
        self.assertEqual([f['key'] for f in pages['home']['fields']],['hero.title','hero.text'])
        research = [f['key'] for f in pages['research']['fields']]
        self.assertIn('section:research:intro',research)
        self.assertIn('collection:framework:description',research)
        self.assertIn('collection:biophysic:name',research)
        protocol = {f['key']: f for f in pages['protocol']['fields']}
        self.assertEqual(protocol['document:doc:meta:title']['value'],'The document')
        self.assertEqual(protocol['document:doc:fact:0:value']['value'],'18h')
        self.assertEqual(protocol['document:doc:section:overview:summary']['value'],'How to read it.')
        self.assertIn('<p>First words.</p>',protocol['document:doc:section:overview:body']['value'])
        self.assertEqual(protocol['document:doc:section:overview:body']['kind'],'rich')
        self.assertEqual(len([k for k in protocol if k.endswith(':body')]),2)

    def test_saving_page_copy_writes_pages_js_with_an_exact_backup(self):
        state = self.make_pages()
        result = self.room.mutate_page(self.page_payload(state,'home',**{'hero.title':'Nothing is<br><em>seen</em> from outside.'}))
        saved = {f['key']: f['value'] for f in __import__('site_pages').read_pages((self.website / 'pages.js').read_bytes())[0]['fields']}
        self.assertEqual(saved['hero.title'],'Nothing is<br><em>seen</em> from outside.')
        self.assertEqual(saved['hero.text'],'One loop.')
        self.assertEqual((self.root/'data'/'backups'/result['pageBackups'][0]).read_bytes(),self.pages_raw)
        self.assertEqual(self.room.content.read_bytes(),self.raw)

    def test_unchanged_page_writes_nothing(self):
        state = self.make_pages()
        result = self.room.mutate_page(self.page_payload(state,'home',**{'hero.title':'Nothing is<br>outside.'}))
        self.assertEqual(result['pageBackups'],[])
        self.assertEqual((self.website / 'pages.js').read_bytes(),self.pages_raw)

    def test_section_and_collection_text_is_written_where_it_lives(self):
        state = self.make_pages()
        self.room.mutate_page(self.page_payload(state,'research',**{'section:research:intro':'Imagination is a beginning.',
                                                                    'collection:framework:description':'The loop, set out.'}))
        section = room_module.read_sections(self.room.section_file)[1][0]
        self.assertEqual((section['id'],section['intro'],section['num']),('research','Imagination is a beginning.','01'))
        framework = room_module.read_collections(self.room.collection_file)[1][0]
        self.assertEqual((framework['description'],framework['order']),('The loop, set out.',['test-article']))
        self.assertEqual((self.website / 'pages.js').read_bytes(),self.pages_raw)

    def test_protocol_edits_rewrite_its_source_and_rebuild_documents_js(self):
        state = self.make_pages()
        self.room.mutate_page(self.page_payload(state,'protocol',**{
            'document:doc:meta:title':'The document, revised',
            'document:doc:fact:0:value':'19h',
            'document:doc:section:body:summary':'Second & last.',
            'document:doc:section:body:body':'<p>Second words, rewritten.</p>'}))
        source = (self.website / 'documents' / 'doc.html').read_text('utf-8')
        self.assertTrue(source.startswith('<!-- A comment that names <section data-id …>'))
        self.assertIn('<p>First words.</p>',source)
        self.assertIn('id="pacer"',source)
        self.assertIn('data-summary="Second &amp; last."',source)
        built = json.loads((self.website / 'documents.js').read_text('utf-8')[len('window.SC_DOCUMENTS = '):].rstrip().rstrip(';'))
        doc = built[0]
        self.assertEqual((doc['title'],doc['facts'][0]['value'],doc['version']),('The document, revised','19h','1.0'))
        self.assertEqual([s['id'] for s in doc['sections']],['overview','body'])
        self.assertEqual(doc['sections'][1]['summary'],'Second & last.')
        self.assertEqual(doc['sections'][1]['html'],'<p>Second words, rewritten.</p>')

    def test_page_refuses_stale_files_foreign_fields_and_page_code(self):
        state = self.make_pages()
        (self.website / 'pages.js').write_bytes(self.pages_raw.replace(b'One loop.',b'Edited elsewhere.'))
        with self.assertRaises(room_module.Conflict):
            self.room.mutate_page(self.page_payload(state,'home',**{'hero.title':'Mine'}))
        state = self.room.state()
        with self.assertRaises(ValueError):
            self.room.mutate_page(self.page_payload(state,'home',**{'section:research:intro':'Not on this page'}))
        with self.assertRaises(ValueError):
            self.room.mutate_page(self.page_payload(state,'home',**{'hero.title':'<script>alert(1)</script>'}))
        with self.assertRaises(ValueError):
            self.room.mutate_page(self.page_payload(state,'nowhere',**{'hero.title':'x'}))
        self.assertIn(b'Edited elsewhere.',(self.website / 'pages.js').read_bytes())

    def test_article_library_still_opens_without_page_files(self):
        state = self.room.state()
        self.assertEqual(state['pages'],[])
        self.assertIn('pagesError',state)
        self.assertEqual(len(state['articles']),1)

    def test_pages_file_is_served_to_the_preview(self):
        self.make_pages()
        self.assertIsNotNone(room_module.safe_public_file(self.website,'pages.js',site=True))
    ...
    def test_http_origin_host_token_and_private_paths(self):
        ...
        try:
            ...
            state = self.make_pages()
            page = room_module.encoded({'page':'home','values':{'hero.text':'Over HTTP.'},'revisions':self.room.state()['pageRevisions']})
            self.assertEqual(request('/api/page-save',page,{**headers,'Origin':'https://evil.example'})[0],403)
            self.assertEqual(request('/api/page-save',page,headers)[0],200)
            self.assertEqual(request('/api/page-save',page,headers)[0],200)   # the same text again changes nothing
            stale = room_module.encoded({'page':'home','values':{'hero.text':'Again.'},'revisions':state['pageRevisions']})
            self.assertEqual(request('/api/page-save',stale,headers)[0],409)
        finally:
            server.shutdown()
            server.server_close()
