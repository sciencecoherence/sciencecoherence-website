...
import site_pages  # noqa: E402  (the main pages' copy: pages.js, sections, collections, the Protocol)
LOCK = threading.RLock()
# Bumped when the local service changes in a way the page depends on, so the
# launcher can tell a stale background service from the current one.
ROOM_VERSION = 4
...
def read_list(path, name, what):
    ...
    if len(ids) != len(items) or any(not isinstance(item, str) for item in ids) or len(set(ids)) != len(ids):
        raise ValueError(f'The website has missing or duplicate {what} addresses.')
    if any(not isinstance(item.get('name'), str) or not item['name'].strip() for item in items):
        raise ValueError(f'Every website {what} needs a name.')
    return raw, items


def read_sections(path):
    """Sections: where a piece lives on the site (Research, The Lab, …). Read-only here."""
    return read_list(path, 'SC_SECTIONS', 'section')


def read_collections(path):
    """Collections: named groups inside a section (Research: Framework, …)."""
    raw, collections = read_list(path, 'SC_COLLECTIONS', 'collection')
    if any(not isinstance(item.get('section'), str) for item in collections):
        raise ValueError('Every website collection must name its section.')
    return raw, collections


def places(sections, collections):
    """Where an article may be filed: each section that accepts articles, with
    the collections it holds. The Protocol is a document, not an article section."""
    allowed = {item['id']: set() for item in sections if item.get('acceptsArticles') is not False}
    for item in collections:
        if item['section'] in allowed:
            allowed[item['section']].add(item['id'])
    return allowed
...
class Room:
    def __init__(self, website, data, port, preview_port):
        ...
        self.section_file = self.website / 'sections.js'
        self.collection_file = self.website / 'collections.js'
        read_articles(self.content)
        read_sections(self.section_file)
        read_collections(self.collection_file)
        self.copy = site_pages.SiteCopy(self.website)
    ...
    def state(self):
        ...
        _, sections = read_sections(self.section_file)
        ...
        return {'articles': articles, 'revisions': {a['id']: revision(a) for a in articles},
                'drafts': self.drafts(), 'sections': sections,
                'collections': collections, 'collectionRevision': digest(collection_raw),
                'previewUrl': self.preview, 'token': self.token, 'online': False, 'mode': 'local',
                'roomVersion': ROOM_VERSION, **self.page_state()}

    def page_state(self):
        try:
            pages, revisions = self.copy.view()
            return {'pages': pages, 'pageRevisions': revisions}
        except (OSError, ValueError) as error:
            # The article library keeps working even if a page file is missing or unreadable.
            return {'pages': [], 'pageRevisions': {}, 'pagesError': str(error) if isinstance(error, ValueError) else 'The page files could not be read.'}

    def mutate_page(self, payload):
        with LOCK:
            stamp = datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S.%fZ')
            def backup(path, raw):
                name = f'{stamp}-{uuid.uuid4().hex[:8]}.{path.name}'
                atomic_write(self.data / 'backups' / name, raw)
                return name
            backups = self.copy.save(payload, atomic_write, backup)
            result = self.state()
    ...
    def mutate_collection(self, action, payload):
        with LOCK:
            ...
            if description is not None and (not isinstance(description, str) or len(description) > 600):
                raise ValueError('Keep the collection description to 600 characters or fewer.')
            _, sections = read_sections(self.section_file)
            ...
            if action == 'collection-create':
                ...
                section = proposed.get('section', 'research')
                if section not in places(sections, collections):
                    raise ValueError('Choose a section of the website that holds articles.')
                numbers = [int(item['num']) for item in collections
                           if item['section'] == section and str(item.get('num', '')).isdigit()]
                collections.append({'id': key, 'section': section, 'num': str(max(numbers, default=0) + 1).zfill(2),
                                    'name': name.strip(), 'description': (description or '').strip(),
                                    'tagline': '', 'connected': [], 'order': []})
            elif action == 'collection-rename':
                if current is None:
                    raise ValueError('That collection could not be found.')
    ...
    def mutate(self, action, payload):
        with LOCK:
            ...
            collection = article.get('collection') or None
            if collection is not None and not isinstance(collection, str):
                raise ValueError('The collection field must contain text.')
            article['collection'] = collection
            if action != 'discard':
                # An article filed where the website has no place for it would
                # break every page that lists it.
                _, sections = read_sections(self.section_file)
                _, collections = read_collections(self.collection_file)
                allowed = places(sections, collections)
                if article['category'] not in allowed:
                    raise ValueError('Choose where this article belongs on the website.')
                if allowed[article['category']] and collection not in allowed[article['category']]:
                    raise ValueError('Choose one of this section’s collections for the article.')
                if not allowed[article['category']] and collection is not None:
                    raise ValueError('That section has no collections; file the article in the section itself.')
            ...
            if merged.get('collection') is None:
...
def safe_public_file(root, relative, site=False):
    ...
    if not parts or any(p.startswith('.') or p == '..' for p in parts):
        return None
    if site and not (relative in ('index.html', 'styles.css', 'app.js', 'sections.js', 'collections.js', 'pages.js', 'content.js', 'documents.js') or parts[0] in ('assets', 'sources')):
        return None
...
def handler(room, preview=False):
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if action not in ('draft', 'apply', 'discard', 'collection-create', 'collection-rename', 'page-save') or not self.path.startswith('/api/'):
                self.response(404, encoded({'error': 'Not found'}))
                return
            try:
                ...
                result = (room.mutate_collection(action, payload) if action.startswith('collection-')
                          else room.mutate_page(payload) if action == 'page-save' else room.mutate(action, payload))
