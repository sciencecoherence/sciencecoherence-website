...
(() => {
  ...
  const RESEARCH_CATEGORIES = [
    {
      ...
      connected: [
        ...
        { href: '#/research/notes-methods', name: 'Notes & Methods' }
      ],
      items: [
        'time-crystalline-v2',
    ...
    },
    {
      ...
      connected: [
        { href: '#/research/framework', name: 'Framework' },
        { href: '#/principles', name: 'Editorial principles' },
        { href: '#/about', name: 'About the inquiry' },
        { href: link('start-here'), name: 'Start here' }
      ],
      items: ['ethos-of-being']
    },
    {
      id: 'meta-mathematics',
      num: '03',
      label: 'Meta-Mathematics',
      ...
      connected: [
        { href: '#/research/framework', name: 'Framework' },
        { href: link('research-method'), name: 'Making imagination a testable reality' },
        { href: '#/lab', name: 'The Lab' }
      ],
      items: []
    },
    {
      ...
      connected: [
        { href: '#/protocol/holographic', name: 'Holographic Biophysics' },
        { href: '#/protocol', name: 'The Protocol' },
        { href: link('regenesis'), name: 'Regenesis' },
        { href: '#/research/framework', name: 'Framework' }
      ],
      items: []
    },
    {
      ...
      connected: [
        ...
        { href: '#/research/framework', name: 'Framework' },
        { href: '#/lab', name: 'The Lab' },
        { href: '#/library', name: 'The Library' }
      ],
      items: ['research-method', 'regenesis', 'start-here']
    }
  ];
  ...
  function renderResearchCategory(cat) {
    const pieces = cat.items.map(id => byId(id)).filter(Boolean);
    let html = '';
    if (pieces.length) {
      html = pieces.map((item, idx) => row(item, idx)).join('');
    } else if (cat.id === 'biophysic') {
      html = `<div class="empty">
        <h3>Work in this area is in progress.</h3>
        <p>The biophysical foundations and exclusion-zone dynamics are being developed.</p>
        <a class="button dark" href="#/protocol/holographic">Explore Holographic Biophysics in The Protocol ↗</a>
      </div>`;
    } else {
      html = `<div class="empty">
        <h3>Work in this area is in progress.</h3>
    ...
    if (cat.id === 'framework') {
      html += `<a class="entry-row" href="#/protocol"><span class="entry-number">↗</span><div><h3>The protocol this leads to</h3><p>The operating document: the three-tier daily protocol, the epigenetic architecture behind it, and the diagnostic suite that reads it back.</p><span class="article-meta">Connected · operating document</span></div><span aria-hidden="true">↗</span></a>`;
    }
    return html;
  }
  ...
  function renderResearchSidebar(cat) {
    ...
    return `<h3>Inside this collection</h3><p>${cat.description || ''}</p><p>${cat.tagline || ''}</p><h3 style="margin-top:28px">Connected paths</h3>${connected}`;
  }
  ...
  function showResearchCategory(catId, { focus = false } = {}) {
    ...
    if (countEl) {
      countEl.textContent = `${cat.items.length} ${cat.items.length === 1 ? 'piece' : 'pieces'}`;
    }
  ...
  function collectionPage(c, selectedCatId) {
    if (c.id === 'research') {
      ...
      const nav = `<div class="doc-nav-wrap"><div class="wrap"><div class="doc-nav" role="tablist" aria-label="Research categories" id="research-nav">${RESEARCH_CATEGORIES.map(cat => `<button role="tab" id="tab-${cat.id}" data-category="${cat.id}" aria-selected="${cat.id === activeCat.id}" aria-controls="research-items"><i>${cat.num}</i>${cat.label}</button>`).join('')}<div class="mode"><span class="research-count" id="research-count">${activeCat.items.length} ${activeCat.items.length === 1 ? 'piece' : 'pieces'}</span></div></div></div></div>`;
  ...
  /* How far across the current inter-rung span a moment sits. Clamped, so the
     first Matrix date (before tau reaches the launch rung at tau = 1) reads 0
     rather than a negative approach. */
  function spanFraction(tau, rung, next) {
    const a = rung.tau, b = next ? next.tau : TAU_STAR;
  ...
  function cal360(date = new Date(), longitude = CAL360.longitude) {
    ...
    /* tau — physical Matrix time. The calendar continues indefinitely. */
    const elapsed = getMatrixNow(date) - getMatrixEpoch();
    const tau     = elapsed / MS_CAL;
    const within  = elapsed - Math.floor(tau) * MS_CAL;
    const cal     = matrixCalendar(tau);
  ...
  const nextRung            = tau => (tau >= TAU_STAR ? null : LADDER.find(L => L.tau > tau) || null);
  ...
  (function checkMatrix() {
    ...
    /* the live descent: exact AT every rung, and it never rises */
    LADDER.forEach((L, i) => {
      const nx = LADDER[i + 1] || null;
      const at = liveLoad(L.tau, L, nx, false);
      if (Math.abs(at - L.J) > 1e-6) f.push(`live J at rung ${i}: ${at} vs ${L.J}`);
    });
    let pl = Infinity;
  ...
  function calendar360(root) {
    ...
    function tick() {
      ...
      el('today').innerHTML =
        `<strong>Matrix date ${bignum(t.dateNumber)}</strong> — `
        ...
        + (t.absolute
            ...
            : `State <strong>${t.state}</strong>, descending from rung <strong>q = ${bignum(t.rung.q)}</strong>. `
