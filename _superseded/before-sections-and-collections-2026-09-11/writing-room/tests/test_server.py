...
class WritingRoomTests(unittest.TestCase):
    def setUp(self):
        ...
        self.collections = [{'id':'research','name':'Research & frameworks','short':'Research','num':'01','symbol':'orbit'},
    ...
    def test_state_exposes_collections_and_version(self):
        state = self.room.state()
        self.assertEqual([c['id'] for c in state['collections']],['research','learning','protocol'])
    ...
    def test_create_collection_appends_with_backup_and_keeps_others(self):
        result = self.room.mutate_collection('collection-create',self.collection_payload(id='meta-mathematics',name='Meta-Mathematics',short='Meta-Math'))
        _, saved = room_module.read_collections(self.room.collection_file)
        self.assertEqual(saved[:3],self.collections)
        self.assertEqual(saved[3]['id'],'meta-mathematics')
        self.assertEqual(saved[3]['name'],'Meta-Mathematics')
        self.assertEqual(saved[3]['short'],'Meta-Math')
        self.assertEqual(saved[3]['num'],'08')
        self.assertEqual((self.root/'data'/'backups'/result['collectionBackup']).read_bytes(),self.collection_raw)
        self.assertEqual(self.room.content.read_bytes(),self.raw)

    def test_create_collection_rejects_duplicate_and_bad_address(self):
        with self.assertRaises(room_module.Conflict):
            self.room.mutate_collection('collection-create',self.collection_payload(id='research',name='Again'))
    ...
    def test_stale_collection_revision_is_refused(self):
        ...
        external = b'window.SC_COLLECTIONS = ' + room_module.encoded(self.collections[:2]) + b';\n'
    ...
    def test_rename_collection_keeps_address_and_other_fields(self):
        self.room.mutate_collection('collection-rename',self.collection_payload(id='learning',name='Learning, openly',short='Openly'))
        _, saved = room_module.read_collections(self.room.collection_file)
        renamed = saved[1]
        self.assertEqual((renamed['id'],renamed['name'],renamed['short']),('learning','Learning, openly','Openly'))
        self.assertTrue(renamed['hidden'])
        self.assertEqual(renamed['num'],'07')

    def test_protocol_and_unknown_collections_cannot_be_renamed(self):
        with self.assertRaises(ValueError):
            self.room.mutate_collection('collection-rename',self.collection_payload(id='protocol',name='Renamed'))
        with self.assertRaises(ValueError):
            self.room.mutate_collection('collection-rename',self.collection_payload(id='missing',name='Renamed'))
        self.assertEqual(self.room.collection_file.read_bytes(),self.collection_raw)

    def test_article_must_use_an_article_collection(self):
        for category in ('protocol','no-such-collection'):
            with self.assertRaises(ValueError): self.room.mutate('apply',self.payload(category=category))
            with self.assertRaises(ValueError): self.room.mutate('draft',self.payload(category=category))
        self.assertEqual(self.room.content.read_bytes(),self.raw)
        self.room.mutate('apply',self.payload(category='learning'))
        self.assertEqual(room_module.read_articles(self.room.content)[1][0]['category'],'learning')
    ...
    def test_new_collection_accepts_articles(self):
        ...
        self.room.mutate('apply',self.payload(category='notes'))
        self.assertEqual(room_module.read_articles(self.room.content)[1][0]['category'],'notes')

    def test_invalid_collections_fail_closed(self):
        self.room.collection_file.write_text('window.SC_COLLECTIONS = [{"id":"a","name":"A"},{"id":"a","name":"B"}];','utf-8')
    ...
    def test_collections_file_is_served_to_the_preview(self):
