...
// Collections come from the website's collections.js through the service, so a
// collection created or renamed here is the same one the website shows.
const siteCollections = () => (state && Array.isArray(state.collections) ? state.collections : []);
const articleCollections = () => siteCollections().filter(item => item.acceptsArticles !== false);
function collectionLabel(id) {
  const item = siteCollections().find(entry => entry.id === id);
  if (!item) return id || 'Writing';
  const name = item.short || item.name;
  return item.hidden ? `${name} · hidden` : name;
}
...
function setBusy(value) {
  busy = value;
  for (const id of ['save','apply','new','discard','new-collection']) $(id).disabled = value;
...
function list() {
  ...
  for (const article of items) {
    ...
    button.innerHTML = `<strong>${escapeHTML(article.title || 'Untitled article')}</strong><small>${escapeHTML(collectionLabel(article.category))}${state.drafts[article.id] ? '<span class="draft-dot">● Draft</span>' : ''}</small>`;
...
async function choose(id, force = false) {
  ...
  renderCollections(current.category);
  $('slug').value = current.id;
  $('slug').readOnly = !!(state.revisions[id] || draft);
  $('collection-label').textContent = collectionLabel(current.category);
...
function renderCollections(selected) {
  ...
  select.replaceChildren(...articleCollections().map(item =>
    new Option(item.hidden ? `${item.name} (currently hidden)` : item.name, item.id)));
  // An article whose collection is not on the website keeps showing it, so
  // opening the article never silently refiles it.
  if (selected && ![...select.options].some(option => option.value === selected)) {
    select.add(new Option(`${selected} (not on the website)`, selected));
  }
...
function syncCollectionActions() {
  ...
  if (rename) rename.disabled = busy || !articleCollections().some(item => item.id === $('category').value);
}
...
function readCollectionFields() {
  ...
  const short = $('collection-short').value.trim() || name;
  if (!name) {notice('Give the collection a name.', true); return null;}
  if (name.length > 120 || short.length > 60) {notice('Keep the name under 120 characters and the short name under 60.', true); return null;}
  return {name, short};
}
...
async function newCollection() {
  ...
  const pending = dialog('Create a collection',
    '<label for="collection-name">Name</label><input id="collection-name" maxlength="120" placeholder="e.g. Meta-Mathematics">'
    + '<label for="collection-short">Short name, for filters and labels</label><input id="collection-short" maxlength="60" placeholder="The name, if left empty">'
    + '<label for="collection-id">Address</label><input id="collection-id" maxlength="80" placeholder="meta-mathematics">'
    + `<p>The collection is added to ${where} straight away, with its own page at <code>#/address</code> and a filter in the Library. `
    + 'The address cannot be changed later. The top menu is edited separately, so the collection does not appear there on its own.</p>',
    'Create collection');
  ...
  if (!fields) return;
  const id = $('collection-id').value.trim() || slugify(fields.name);
  ...
  if (!(await collectionRequest('collection-create', {id, ...fields}))) return;
  renderCollections(id);
  $('collection-label').textContent = collectionLabel(id);
  markDirty();
  list();
  notice(`“${fields.name}” is on ${where}. This article is now filed under it — save or update the article to keep that.`);
}

async function renameCollection() {
  if (busy || !state) return;
  const item = articleCollections().find(entry => entry.id === $('category').value);
  if (!item) return;
  const pending = dialog('Rename this collection',
    `<label for="collection-name">Name</label><input id="collection-name" maxlength="120" value="${escapeHTML(item.name)}">`
    + `<label for="collection-short">Short name, for filters and labels</label><input id="collection-short" maxlength="60" value="${escapeHTML(item.short || '')}">`
    + `<p>Its address stays <code>#/${escapeHTML(item.id)}</code>, so existing links keep working. Every article in it moves with it. `
    + 'The top menu is edited separately and keeps its current wording.</p>',
    'Rename collection');
  $('collection-name').focus();
  if (!(await pending)) return;
  const fields = readCollectionFields();
  if (!fields) return;
  if (fields.name === item.name && fields.short === (item.short || item.name)) return;
  if (!(await collectionRequest('collection-rename', {id:item.id, ...fields}))) return;
  renderCollections(item.id);
  $('collection-label').textContent = collectionLabel(item.id);
  list();
  renderPreview();
  notice(`The collection is now called “${fields.name}”.`);
}
...
for (const key of ['title','subtitle','description','category','type','date','slug']) $(key).addEventListener('input', () => {
  markDirty();
  $('collection-label').textContent = collectionLabel($('category').value);
  syncCollectionActions();
