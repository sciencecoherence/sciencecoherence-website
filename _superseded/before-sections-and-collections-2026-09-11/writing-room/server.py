...
def article_collections(collections):
    """Collections an article may belong to. The Protocol is a document, not an article collection."""
    return {item['id'] for item in collections if item.get('acceptsArticles') is not False}
...
class Room:
    ...
    def mutate(self, action, payload):
        with LOCK:
            ...
            if action != 'discard':
                # An article filed under a collection the website does not know
                # would break every page that lists it.
                _, collections = read_collections(self.collection_file)
                if article['category'] not in article_collections(collections):
                    raise ValueError('Choose one of the website’s collections for this article.')
