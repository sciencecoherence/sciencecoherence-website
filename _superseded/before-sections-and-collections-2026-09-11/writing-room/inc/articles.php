...
function wr_mutate(PDO $db, array $user, string $action, array $payload): array {
    ...
    // An article filed under a collection the website does not know would break
    // every page that lists it. The Protocol is a document, not an article collection.
    [, $collections] = wr_read_collections();
    $allowed = [];
    foreach ($collections as $collection) {
        if (($collection['acceptsArticles'] ?? true) !== false) $allowed[] = $collection['id'];
    }
    if (!in_array($article['category'], $allowed, true)) {
        throw new InvalidArgumentException('Choose one of the website’s collections for this article.');
    }
