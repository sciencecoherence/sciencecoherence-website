...
Under **Article details**, **＋ New collection** adds a collection (a section of the website) and files the open article under it; **Rename collection** changes the name and short name of the selected one. Both write the website's `collections.js` at once, after an exact backup, and refuse to overwrite a change made in another window.

- A new collection gets its own page at `#/address`, a Library filter, and search. It does not appear in the top menu, which is written by hand in `index.html`; renaming does not change the menu's wording either.
- A collection's address is fixed once created, so links keep working. Reordering, hiding, and deleting collections are done by editing `collections.js`.
- The Protocol is a document, not an article collection, and cannot be chosen or renamed here.
- An article can only be saved into a collection the website knows, so an article never points at a missing section.
...
Use `python server.py --website PATH --data PATH --port 8875 --preview-port 8876` for an isolated test copy. Do not run write tests against the real website. The tests exercise draft persistence, exact backups, field preservation, conflicts, new articles, no-op saves, collection creation and renaming, article-collection validation, and HTTP access boundaries. The page recognises the local service only at port 8765, so use the isolated ports for the tests and API checks, not for trying the page itself.
