...
collections.js          the collections: names, order, flags (hidden, standalone)
...
sitting inside a collection — `collections.js` marks it `standalone: true`, which
...
`category` is the `id` of a collection in `collections.js` — currently
`research`, `lab`, `transmissions`, `regenesis` or `learning`. `protocol` is
marked `acceptsArticles: false`: it holds the operating document, not articles.

The collections live in `collections.js` as `window.SC_COLLECTIONS`, one object
per collection (`id`, `name`, `short`, `num`, `symbol`, `description`, `intro`,
`label` and the flags below). The Writing Room can create a collection and
rename one; it cannot change an address, reorder, hide or delete — edit the file
for those. `app.js` keeps a copy as a fallback for a page opened without the
file, so keep the two in step. The top navigation in `index.html` is written by
hand and does not follow renames or new collections.
