...
## Decision — collections editable from the Writing Room (2026-09-11)

Started by ChatGPT (Codex) and interrupted when its usage ran out; completed by
Claude the same day.

The collection definitions moved out of `app.js` into `collections.js`
(`window.SC_COLLECTIONS`), loaded before `content.js`. `app.js` keeps its copy
only as a fallback for a page opened without the file. The Writing Room can now
**create** a collection and **rename** one, locally (`server.py`) and online
(`api.php`, a fourth table `sc_wr_collections`). Every change is checked against
the file's revision, backed up exactly, and refused on conflict.

Consequences to preserve:

- A collection's address (`id`) is fixed once created; renaming changes only
  `name` and `short`. Reorder, hide and delete stay manual edits of the file.
- `protocol` carries `acceptsArticles: false`. It cannot be renamed or chosen
  for an article, because it holds the operating document.
- An article can be saved only into a collection that exists and accepts
  articles, so no article points at a missing section.
- The top navigation in `index.html` remains editorial and hand-written. New
  collections get a route, a Library filter and search, not a menu entry.
- The local launcher reuses a running service only when its `roomVersion`
  matches, and otherwise stops that stale Python service and starts the current
  one. Bump `ROOM_VERSION` in `server.py` and `$roomVersion` in
  `Start-WritingRoom.ps1` together.
